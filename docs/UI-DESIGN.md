...
**FIZZ/SPEED/POWERゲージの役割と実装状況**:

**実装状況**:
- **FIZZ値**: 実装済み。スコア計算に影響（`ShakeManager.swift`の`fizzModifier`で使用）
  - FIZZ値が高いほど、同じ振り強度でも圧力の蓄積が速い（スコアが高くなる傾向）
- **SPEED値**: 実装されていない。現在はUI表示のみで、ゲームプレイには影響しない
- **POWER値**: 実装されていない。現在はUI表示のみで、ゲームプレイには影響しない

**パワーゲージが実装されている理由（推測）**:
1. **視覚的な差別化**: 各缶の個性を表現し、選択の動機付け
2. **将来的な拡張性**: 将来的にゲームメカニクスに組み込む予定だったが、まだ実装されていない
3. **UI装飾**: ゲームプレイには影響しないが、見た目の情報量を増やすため

**改善案（レビュアー指摘対応）**:
- **問題点**: 「85%」「70%」は分かるが、*何が85%なのか*が一瞬では伝わらない。また、SPEED/POWERが実際のゲームプレイに影響しないことが不明確
- **改善案C**: 
  - SPEED/POWERを削除し、FIZZのみを表示
  - 各缶の特性は別の方法で表現（例: 缶のデザイン、色、説明文）

**実装判断**: 
- **MVPでは改善案AまたはCを推奨**（実装されていない機能を表示しない方がユーザーの混乱を避けられる）
- SPEED/POWERを将来的に実装する場合は、実装時に追加する

### 自己ベスト表示（YOUR BEST）

**コンセプト**: ユーザーのモチベーションを高めるため、自己ベスト記録を表示

- **セクションタイトル**: 「YOUR BEST」（ネオンシアン・太字・小さめ）
- **ベストスコア**: 大きく表示（例: **18.7m**）— ネオンシアン・太字
- **達成情報**: スコアの下に小さく「Rank A · ULTRA COLA · 2 days ago」など（グレー・細字）
- **未プレイ時**: 「—」または「START YOUR FIRST GAME!」と表示

**配置**: ヘッダー（SHAKE FIZZ タイトル）の直下、飲み物グリッドの上

**MVP 対応**: 初期実装ではローカルストレージ（UserDefaults）に保存した最高スコアを表示。バックエンド実装後はFirestoreのUser.bestScoreから取得。
...
**ゲーム操作ボタン**: シェイク操作とタップ操作の両方に対応。アクセシビリティと直感的な操作を両立。

#### 操作ボタン（画面下部）

**1. タップボタン（シェイク代替）**
- **配置**: 画面下部左側（または中央）
- **ラベル**: **「TAP TO BUILD PRESSURE」** または **「TAP TO SHAKE」**（白・太字・18-20pt）
- **役割**: 
...
**注**: POP THE TOPボタンは実装しない。15秒間のタイムアタック終了後、自動的に圧力が解放され、結果が決定される。

#### タイマー表示（画面下部中央）

- **経過時間 / 残り時間**: 「00:12 / 00:03」形式（モノスペースフォント・白・太字）
- **フォントサイズ**: 18-20pt
- **背景**: 半透明の黒（rgba(0,0,0,0.5)）、角丸 8pt
- **パディング**: 横 16pt、縦 8pt
- **残り時間の視覚的フィードバック強化**（レビュアー指摘対応）:
  - 残り3秒で: 色が赤に変化、振動（ハプティクス）、泡が荒れる
  - 残り5秒で: 黄色に変化（警告）
  - **ゲージ表現の追加**: タイマーの上に円形プログレスバーを表示（残り時間を視覚化）
...
**ゲームフロー詳細:**
1. 飲み物選択 → START SHAKING
2. 安全警告 → GOT IT!
3. カウントダウン（3・2・1）→ 自動でゲーム開始
4. 15秒間のタイムアタック（制限時間内は振り続ける）
5. TIME UP! 自動表示 → 「結果を見る」ボタンで結果画面へ
6. 結果画面 → TRY AGAIN（同じ飲み物で再プレイ）or CHANGE DRINK（飲み物選択へ）
